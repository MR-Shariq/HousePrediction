...
def get_estimated_price(location, sqft, bhk, bathrooms):
    try:
    ...
    sample = np.concatenate((np.array([sqft, bhk, bathrooms]), loc_array))

    return round(model.predict(sample.reshape(1, -1))[0], 2)
